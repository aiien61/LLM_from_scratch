...
if __name__ == "__main__":
    ...
    probas = torch.softmax(outputs, dim=1)  # uses softmax to get the prob of each class
    ic(probas)

    # Opotion 1 to match predicted samples with labels
    predictions = torch.argmax(probas, dim=1)
    ic(predictions)

    # Opotion 2 to match predicted samples with labels
    predictions = torch.argmax(outputs, dim=1)
    ic(predictions)

    ic(predictions == data['y_train'])

    # how many predictions are correct
    ic(torch.sum(predictions == data['y_train']))
